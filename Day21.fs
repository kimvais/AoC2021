...
let repeating max startPos =
    Seq.initInfinite (fun i -> (i + startPos - 1) % max + 1)
...
let roll3 d =
    let numbers = [d+3; d; d+1; d+2] |> Seq.map (fun n -> (n - 1) % 100 + 1)
...
let rec play game =
    ...
    let score, game' =
        ...
        | 1 ->
                ...
                score, {game with Dice=dice; Pos1=pos; Score1=score; Rolls=rolls}
        | 0 ->
                let pos = (game.Pos2 + roll - 1) % 10 + 1
                let score = (game.Score2 + pos)
                score, {game with Dice=dice; Pos2=pos; Score2=score; Rolls=rolls}
...
let day21 sp1 sp2 () =
    let game = {Dice=1; Pos1= sp1; Pos2=sp2; Score1=0; Score2=0; Rolls=0}
    play game |> printfn "%A"
    0L
